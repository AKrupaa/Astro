...
public class DayAfterTomorrowFragment extends Fragment {
    ...
    public DayAfterTomorrowFragment() {
        // Required empty public constructor
    }

    public static DayAfterTomorrowFragment newInstance() {
        DayAfterTomorrowFragment fragment = new DayAfterTomorrowFragment();
        return fragment;
    }
    ...
    private void initialization(SharedViewModel model) {
        ...
        timeText = model.getThirdDay().getAsString(DatabaseHelper.DT_TXT);
        temperature = model.getThirdDay().getAsInteger(DatabaseHelper.TEMPERATURE);
        tempMin = model.getThirdDay().getAsInteger(DatabaseHelper.TEMP_MIN);
        tempMax = model.getThirdDay().getAsInteger(DatabaseHelper.TEMP_MAX);
        pressure = model.getThirdDay().getAsInteger(DatabaseHelper.PRESSURE);
        humidity = model.getThirdDay().getAsInteger(DatabaseHelper.HUMIDITY);
        seaLevel = model.getThirdDay().getAsInteger(DatabaseHelper.SEA_LEVEL);
        grndLevel = model.getThirdDay().getAsInteger(DatabaseHelper.GRND_LEVEL);
        weatherMain = model.getThirdDay().getAsString(DatabaseHelper.WEATHER_MAIN);
        weatherDescription = model.getThirdDay().getAsString(DatabaseHelper.WEATHER_DESCRIPTION);
        weatherIcon = model.getThirdDay().getAsString(DatabaseHelper.WEATHER_ICON);
        windSpeed = model.getThirdDay().getAsInteger(DatabaseHelper.WIND_SPEED);
        windDeg = model.getThirdDay().getAsInteger(DatabaseHelper.WIND_DEG);
    }
